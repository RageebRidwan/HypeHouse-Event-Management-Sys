...
async function main() {
  ...
  const participants = await Promise.all([
    ...
    // Tech Networking (Paid) - 12 participants with payment
    prisma.participant.create({
      data: {
        userId: users[2].id,
        eventId: events[1].id,
  ...
  // Update event currentCount based on participants
